...
class DataSetImages(object):
    ...
    def __init__(self):
        self.size_min = 4 * 20  # This number must be multiple of 4
        self.file_pickle = 'data/lfw/lfw.pickle'
        self.file_img_list = 'data/lfw/name2gender.txt'
    ...
    def preprocess_images(self):
        ...
        for index, row in dfimg.iterrows():
            ...
            # Only use images with valid 'sex' attribute
            if img_sex != 'male' and img_sex != 'female':
                print('Skipping image ', img_file, ', having sex ', img_sex)
                continue

            # Code sex form {'male', 'female'} to {True, False}
            img_sex = (img_sex == 'male')
            ...
            if dim[0] != dim[1]:
                print('WARNING: Image ', img_file, ' is not square, skipping:',
                      ', size:', im.shape)
                continue

            size_im = dim[0]
            if size_im < self.size_min:
                print('WARNING: Image ', img_file, 'is too small, skipping:',
                      ', size:', im.shape)
                continue
